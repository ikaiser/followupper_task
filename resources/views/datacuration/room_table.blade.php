...
@section('content')
    ...
    <div class="row">
        <div class="col s6">
            <h6>
                ...
                @foreach($dcs as $single_dc)
                    <a onclick="document.location.href='{{ route('dc.get', [$project->id, $single_dc->id]) }}'">&nbsp;/ {{$single_dc->name}}&nbsp;</a>
                @endforeach
            </h6>
        </div>
        <div class="col s6 right-align">
            ...
            @endif
            <button class="btn btn-floating waves-effect waves-light" onclick="document.location.href='{{route('dc.get', [$project->id, $dc->id])}}'" title="Cambia Layout"><i class="material-icons">grid_on</i></button>
                    ...
                    <table class="table-responsive highlight display" id="dc_table">
                        ...
                        @foreach($rooms as $room)
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-small waves-effect waves-light" onclick="document.location.href='{{route('dc.get', [$project->id, $room->id])}}'" title="view"><i class="material-icons">open_in_browser</i></button>
                                    @if(\Illuminate\Support\Facades\Auth::user()->roles->first()->id < 4)
                                        <button type="button" class="btn btn-small waves-effect waves-light" onclick="document.location.href='{{route('dc.users', [$project->id, $room->id])}}'" title="assign user"><i class="material-icons">folder_shared</i></button>
                                        <button type="button" class="btn btn-small waves-effect waves-light" onclick="document.location.href='{{route('dc.edit', [$project->id, $room->id])}}'" title="edit"><i class="material-icons">edit</i></button>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
