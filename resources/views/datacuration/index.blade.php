    ...
    <div class="row">
        ...
        <div class="col s12 m6 mt-4 right-align">
            ...
            @if(\Illuminate\Support\Facades\Auth::user()->roles->first()->id < 4)
                <button class="btn btn-floating waves-effect waves-light" onclick="document.location.href='{{ route('dc.create_index', $project->id) }}'" title="@lang('Add Room')"><i class="material-icons">folder_open</i></button>
                <button class="btn btn-floating waves-effect waves-light" onclick="document.location.href='{{ route('dce.create_index', $project->id) }}'" title="@lang('Add File')"><i class="material-icons">insert_drive_file</i></button>
            @endif
            <!-- <button class="btn btn-floating waves-effect waves-light" onclick="document.location.href='{{ route('dc.index', $project->id) }}?layout=table'" title="Cambia Layout"><i class="material-icons">format_list_bulleted</i></button> -->
                            ...
                            @foreach($dcs as $dc)
                                <div class="col s12 l4 m4">
                                    <div class="card mb-4 hoverable">
                                        <div style="background-image: url('{{Storage::url('dc/') . $dc->thumbnail}}'); height: 150px; width: 100%; background-position: center;background-size: cover; background-repeat: no-repeat;"></div>
                                        <div class="card-content">
                                            <span class="card-title truncate" title="{{$dc->name}}">{{$dc->name}}</span>
                                            <div class="mt-2">
                                                <button type="button" class="btn btn-small waves-effect waves-light m-1" style="padding: 0 15px" onclick="document.location.href='{{route('dc.get', [$project->id, $dc->id])}}'"> @lang('View') </button>
                                                @if(\Illuminate\Support\Facades\Auth::user()->roles->first()->id < 4)
                                                    <button type="button" class="btn btn-small waves-effect waves-light m-1" style="padding: 0 15px" onclick="document.location.href='{{route('dc.users', [$project->id, $dc->id])}}'"> @lang('Assign Users') </button>
                                                    <button type="button" class="btn btn-small waves-effect waves-light m-1" style="padding: 0 15px" onclick="document.location.href='{{route('dc.edit', [$project->id, $dc->id])}}'"> @lang('Edit') </button>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-content">
                                            ...
                                            <div class="row mt-3">
                                                ...
                                                <div class="col s6 mt-1 right-align">
                                                    ...
                                                    @if(\Illuminate\Support\Facades\Auth::user()->roles->first()->id < 4)
                                                        <span class="material-icons ml-10 pointer" onclick="document.location.href='{{route('dce.edit', [$project->id, $file->id])}}'" >edit</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
